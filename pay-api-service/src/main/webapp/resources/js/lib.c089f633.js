...
function JDP() {
    this.ua = navigator.userAgent, this.android = this.ua.match(/(Android)\s+([\d.]+)/), this.ios = this.ua.match(/(iPhone\sOS)\s([\d_]+)/)
}
function init() {
    safeCall(function () {
        WeixinJSBridge.call("hideOptionMenu"), WeixinJSBridge.call("hideToolbar")
    }), safeCall(function () {
        jdp.setTitle($("title").html())
    })
}
function pay(a) {
    ...
    WeixinJSBridge.invoke("getBrandWCPayRequest", {
        ...
        "get_brand_wcpay_request:ok" == b.err_msg ? function (){
            $("#amount").test("");
            window.location = baseURL+"/register/pay_succeed/"+orderId+".mvc";
        } : "get_brand_wcpay_request:cancel" == b.err_msg || WeixinJSBridge.invoke("closeWindow", {}, function (a) {
            $("#amount").test("");
        })
    })
}
function safeCall(a) {
    "undefined" == typeof WeixinJSBridge ? document.addEventListener ? document.addEventListener("WeixinJSBridgeReady", a, !1) : document.attachEvent && (document.attachEvent("WeixinJSBridgeReady", a), document.attachEvent("onWeixinJSBridgeReady", a)) : a(), window.WebViewJavascriptBridge ? a() : document.addEventListener("WebViewJavascriptBridgeReady", a, !1)
}
function pay_now(a) {
    console.log(a);
    var b = {};
       if($("#open_code").val()!=""&&$("#open_code").val()!=undefined&&
           $("#userid").val()!='ffa4f8c5-f8cc-4a31-9d4a-cbf0765cec42'&&$("#agent_name").val()!='CK卡'){
           b.amt = $("#amt").val(),  b.userid = $("#userid").val(), b.code = $("#open_code").val(),b.openid=$("#openid").val(), $.ajax({
               url: location.origin + "/remotePay/sdj_nayouyigpay",
               type: "get",
               dataType: "json",
               data: b,
               beforeSend: function () {
               ...
               success: function (a) {
                   if ("0001" == a.respcd) {
                       alert(a.resperr);
                   }else if(a.respcd=="0002"){
                       window.location = location.origin + "/register/offline_pay.mvc" + "?code_url=" + a.codeUrl + "&pay_name=微信支付";
                   }else if (a.respcd == "0003") {
                       alert("请用微信或者支付宝打开");
                   }
                    else safeCall(function () {
                           $("#payAmount").val(a.data.payAmount);
                           $("#platformCode").val(a.data.platformCode);
                           $("#interfaceVersion").val(a.data.interfaceVersion);
                           $("#signType").val(a.data.signType);
                           $("#code").val(a.data.code);
                           $("#serviceType").val(a.data.serviceType);
                           $("#sign").val(a.data.sign);
                           $("#inputCharset").val(a.data.inputCharset);
                           $("#orderNo").val(a.data.orderNo);
                           $("#payeeId").val(a.data.payeeId);
                           $("#clientIp").val(a.data.clientIp);
                           $("#notifyUrl").val(a.data.notifyUrl);
                           $("#signature").val(a.data.signature);
                           $("#productName").val(a.data.productName);
                           add();
               ...
               error: function (a) {
                   $("#loading").hide(), $("#pay").addClass("active"), alert("网络不好， 请稍后重试!!!"), safeCall(function () {
           ...
           b.amt = $("#amt").val(), b.openid = $("#openid").val(), b.sub_openid = $("#sub_openid").val(), b.userid = $("#userid").val(), b.opuid = $("#opuid").val(), b.discount = $("#discount").val(), $.ajax({
               url: location.origin + "/remotePay/nayouyigpay",
               type: "get",
               dataType: "json",
               data: b,
               beforeSend: function () {
               ...
               success: function (a) {
                   if ("0001" == a.respcd) {
                       alert(a.resperr);
                       //alert(a.resperr),
                       //    safeCall(function () {WeixinJSBridge.invoke("closeWindow", {}, function (a) {
                       //    })
                       //}), safeCall(function () {
                       //    jdp.close()
                       //}
                       ...
                       if ("weixin" == a.data.type) {
                           window.location = location.origin + "/register/offline_pay.mvc" + "?code_url=" + a.data.codeUrl + "&pay_name=微信支付";
                       } else if ("alipay" == a.data.type) {
                           window.location = location.origin + "/register/offline_pay.mvc" + "?code_url=" + a.data.codeUrl + "&pay_name=支付宝支付";
                       }
                   } else if (a.respcd == "0003") {
                       alert("请用微信或者支付宝打开");
                   }else if(a.respcd == "0004"){
                       //直接跳转到支付宝支付
                       ...
                       pay(a)
                   }); else"alipay" == a.data.type ? window.location.replace(a.data.pay_url) : "jdpay" == a.data.type && jdp.pay({
                       params: a.data.pay_params,
                       callback: function (a) {
                           if (a) {
                               var b = JSON.parse(a);
                               "SUCCESS" == b.status && jdp.close()
                           }
                       }
                   })
               },
    ...
    return function (a) {
        ...
        e.prototype = {
            nameLookup: function (a, b) {
                ...
                var l = {compiler: this.compilerInfo(), main: k}, m = this.context.programs;
        ...
        d.prototype = {
            constructor: d, mutating: !1, acceptKey: function (a, b) {
        ...
        var c = function () {
            ...
            var b = {
                ...
                performAction: function (a, b, c, d, e, f) {
                    ...
                    switch (e) {
                        ...
                        case 40:
                            ...
                            break;
                        case 41:
                            f[g - 2].push({part: d.id(f[g]), original: f[g], separator: f[g - 1]}), this.$ = f[g - 2];
                            break;
                        case 42:
                            this.$ = [{part: d.id(f[g]), original: f[g]}];
                            break;
                        case 43:
                            this.$ = [];
                            break;
                ...
                return a.options = {}, a.performAction = function (a, b, c, d) {
                    ...
                    INITIAL: {rules: [0, 1, 42], inclusive: !0}
                }, a
            }();
        ...
        g || (g = function (a, b, c, d) {
            ...
            prepend: function (a, b) {
                ...
                var a = void 0 === arguments[0] ? this.currentLocation || {start: {}} : arguments[0];
                return new g(a.start.line, a.start.column, this.srcFile)
            }, wrap: function (a) {
                var b = void 0 === arguments[1] ? this.currentLocation || {start: {}} : arguments[1];
...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto), function (a) {
    ...
    constructor: JDP, getInfo: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, "");
        this.fnCallback(b, a), this.android ? android.getInfo(b) : this.openIframe("native://getInfo?callback=" + b)
    }, setTitle: function (a) {
        //this.android ? this.android.setTitleName(a) : this.openIframe("native://setTitleName?titleName=" + a)
    }, alert: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, "");
        this.android ? android.alert(a) : (this.fnCallback(b, null), this.openIframe("native://alert?text=" + a + "&callback=" + b))
    }, confirm: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, "");
        this.fnCallback(b, a.callback), this.android ? android.confirm(a.text, a.sure, a.cancel, b) : this.openIframe("native://confirm?text=" + a.text + "&btn0Name=" + a.sure + "&btn1Name=" + a.cancel + "&callback=" + b)
    }, close: function () {
        this.android ? android.close() : location.href = "native://close"
    }, addCache: function (a) {
        this.android ? android.putCache(a.key, a.value) : this.openIframe("native://putCache?key=" + a.key + "&value=" + a.value)
    }, getCache: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, "");
        this.fnCallback(b, a.callback), this.android ? android.getCache(a.key, b) : this.openIframe("native://getCache? key=" + a.key + "&callback=" + b)
    }, removeCache: function (a) {
        this.android ? android.removeCache(a.key) : this.openIframe("native://removeCache? key=" + a.key)
    }, removeAllCache: function () {
        this.android ? android.removeAllCache() : this.openIframe("native://removeAllCache")
    }, goBackListener: function (a) {
        window.goBackListenerCallback = function () {
            return a ? a() : 0
        }, this.android ? android.setGoBackListener("goBackListenerCallback") : this.openIframe("native://setGoBackListener?listener=goBackListenerCallback")
    }, checkNetwork: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, "");
        this.fnCallback(b, a), this.android ? android.checkNetwork(b) : this.openIframe("native://checkNetwork?callback=" + b)
    }, buriedPoint: function (a) {
        var b = a.once ? !0 : !1;
        b && this.android ? android.buriedPointOnce(a.eventId) : android.buriedPoint(a.eventId, a.eventLable), b && this.ios ? this.openIframe("native://buriedPoint?eventId=" + a.eventId) : this.openIframe("native://buriedPoint?eventId=" + a.eventId + "&eventLable=" + a.eventLable)
    }, openAppModel: function (a) {
        var b = this.json2str(a.params) || "", c = a.name.toUpperCase(), d = ("globalCallback" + Math.random()).replace(/\./, "");
        return this.fnCallback(d, a.callback), "ACTIVITY" == a.name && this.android ? void android.start(a.params.url, d) : "ACTIVITY" == a.name && this.ios ? void this.openIframe("native://start?name=" + a.params.url + "&callback=" + d) : void(this.android ? android.start(c, JSON.stringify(a.params), d) : this.openIframe("native://start?name=" + c + b + "&callback=" + d))
    }, shareText: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, ""), c = a.channel || "";
        this.fnCallback(b, a.callback), this.android ? android.shareText(a.text, c, b) : this.openIframe("native://shareText?text=" + a.text + "&jsonChannel=" + c + "&callback=" + b)
    }, shareImage: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, "");
        a.channel || "";
        this.fnCallback(b, a.callback), this.android ? android.shareImage(a.startY, a.endY, a.channel, b) : this.openIframe("native://shareImage?startY=" + a.startY + "&endY=" + a.endY + "&jsonChannel=" + a.channel + "&callback=" + b)
    }, shareUrl: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, ""), c = a.channel || "";
        this.fnCallback(b, a.callback), this.android ? android.shareWebPage(a.url, a.title, a.desc, c, b) : this.openIframe("native://shareWebPage?url=" + a.url + "&webTitle=" + a.title + "&webDesc=" + a.desc + "&jsonChannel=" + c + "&callback=" + b)
    }, json2str: function (a) {
        var b = "";
        ...
        window[a] = function (c) {
            b && b(c), delete window[a]
        }
    }, setMenu: function (a) {
        var b = {};
        str = "[", a.forEach(function (a, c) {
            b["item" + c] = a, str += JSON.stringify(a) + ","
        }), str = str.substring(0, str.length - 1), str += "]", this.android ? android.setTitleMenu(str) : WebViewJavascriptBridge.callHandler("setMenu", b)
    }, pay: function (a) {
        var b = ("globalCallback" + Math.random()).replace(/\./, ""), c = JSON.stringify(a.params);
        this.fnCallback(b, a.callback), delete a.callback, this.android ? android.pay(c, b) : this.openIframe("native://commonPay?params=" + c + "&callback=" + b)
    }
};
var jdp = new JDP;
!function (a) {
    ...
    b.prototype.insert = function (a, b, c) {
        ...
        a.removeClass("show"), $(".amount").removeClass("active")
    }, b.prototype.submitData = function (a) {
        "" != a.text() && a.text() > 0 && $("#pay").hasClass("active") && ($("#amt").val((100 * a.text()).toFixed(0)), pay_now(a))
        ...
        $("#keyBoard").addClass("show"), $(this).addClass("active")
    }), a.keyBoard = b
}(window);
var kboard = new keyBoard("keyBoard", "amount", {"int": "5", decimal: "2"});
setTimeout(function () {
    $("#keyBoard").addClass("show-trans").ready(function () {
        $("#keyBoard").addClass("show")
    })
}, 500), Handlebars.registerHelper("compare", function (a, b, c) {
