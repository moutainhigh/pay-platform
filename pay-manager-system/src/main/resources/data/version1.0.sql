...
create table tb_merchant(
	id char(36) primary key ,
	merchant_no varchar(50) UNIQUE comment '商家编号',
	merchant_name varchar(255) comment '商户名称' ,
	phone varchar(255) comment '手机号',
	real_name varchar(255) comment '真实姓名',
	identity_code varchar(255) comment '身份证号码',
	id_card_img1 varchar(500) comment '身份证-正面',
	id_card_img2 varchar(500) comment '身份证-反面',
	icp_img varchar(500) comment 'icp证件',
	merchant_secret varchar(500) comment '商家密钥',
	check_status varchar(50) default 'waitCheck'  comment '审核状态(waitCheck:待审核 success:审核通过 fail:审核失败)' ,
	check_desc varchar(500) comment '审核备注',
	`is_del` tinyint(4) DEFAULT '0' COMMENT '是否删除(0:否 1:是)',
	create_time datetime comment '创建时间'
) comment '商家';
...
create table tb_order(
    ...
    pay_status varchar(50) default 'waitPay' comment '支付状态(waitPay:待支付 payed:已支付 fail:支付失败)',
	pay_time datetime comment '支付时间' ,
	notify_url varchar(500) comment '商户回调地址' ,
	notify_status varchar(500) comment '回调商户状态(notNotify:未通知 notifyed:已通知 success:通知成功,收到了商户反馈)' ,
	notifiy_num int default 0 default 0 comment '通知次数' ,
	create_time datetime comment '创建时间'
) comment '订单';
