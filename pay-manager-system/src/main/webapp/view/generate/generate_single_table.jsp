...
<script type="text/javascript">
    ...
    var defaultAddField = ["id", "createTime", "isDel"];
    ...
    dbTable = $('#dbTable').initBootstrapTable({
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    if (row.fieldName == "id") {
                        return "<input type='checkbox' checked disabled class='generate-checkbox' name='primaryKeyRow' />";
                    } else {
    ...
    viewTable = $('#viewTable').initBootstrapTable({
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function (value, row, index) {
                    if (row.fieldName == "id") {
                        return "<input type='checkbox' id='selectAllShowList' name='_isShowList' class='generate-checkbox' />";
                    } else {
                        return "<input type='checkbox' name='_isShowList' class='generate-checkbox' />";
                    }
                }
            },
            {
    ...
    $(function () {
        ...
        //控制全选/全不选
        $(document).on("click", "#selectAllShowList", function () {
            var checked = this.checked;
            $("input[name='_isShowList']").prop("checked", checked);
        });
     ...
     * 生成单表代码
     */
    function generateSingleTableCode() {
        ...
        $("#viewTable input[name='_isShowList']").each(function (index, obj) {
            if (index != 0) {
                var remarks = $(this).parent().parent().find("[name='remarks']").val();
                columnList[index].remarks = remarks;

                var _isShowList = $(this).parent().parent().find("[name='_isShowList']").prop("checked") || false;
                showListCount = (_isShowList ? (showListCount + 1) : showListCount);
                columnList[index]._isShowList = _isShowList;
        ...
        $.ajax({
            url: baseURL + "/generate/generateSingleTableCode?_csrf=" + token,
            type: "post",
            dataType: "json",
            async: true,
            data: params,
    ...
    <div class="col-md-12">
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                <button id="add" class="btn btn-success" onclick="generateSingleTableCode()" style="display: block;">
